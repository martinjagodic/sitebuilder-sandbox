...
While this project still lives in `sitebuilder-sandbox`, run it from the sandbox root instead — `./sb dev planet-plus` — so Hugo uses the local core checkout.
...
| Address, phone, hours, social links | `data/business.json` (header, footer, location module, search engines) |
